...
from app.llm import Provider, Tier, ChatMessage, LLMClient, MODEL_REGISTRY
...
async def _resolve_client(user_id: str = "") -> LLMClient:
    ...
    if client is None:
        raise HTTPException(
            status_code=400,
            detail="LLM not configured. Set API keys in .env or POST /llm/config",
        )
    return client
...
async def set_config(req: ConfigRequest, x_user_id: str = Header(alias="X-User-Id", default="")):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unknown provider: {req.provider}. Options: {[p.value for p in Provider]}",
        )
