...
@router.post("/ask", response_model=AgentResponse, summary="Ask the agent a question")
async def ask_agent(
    query: AgentQuery, agent: AgentService = Depends(get_agent_service)
) -> AgentResponse:
    """
    ...
    try:
        ...
        logger.info(
            f"Agent query processed successfully. Tools used: {result.get('tools_used', [])}"
        )
        return AgentResponse(**result)
    except Exception as e:
        logger.error(
            f"Error processing agent query '{query.query}': {e}", exc_info=True
        )
        raise HTTPException(
...
@router.post("/ask/stream", summary="Ask the agent with streaming response")
async def ask_agent_stream(
    ...
    try:
        ...
        async def event_generator():
            ...
            try:
                ...
                async for event in agent.process_query_stream(query.query):
                    ...
                    event_count += 1
                logger.info(
                    f"Streaming query completed. Generated {event_count} events"
                )
            except Exception as e:
                logger.error(
                    f"Error in stream for query '{query.query}': {e}", exc_info=True
                )
                error_event = {
                    "type": "error",
        ...
        logger.error(
            f"Error setting up stream for query '{query.query}': {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
